...
#conductor_codis3<-here::here("Projecte_HTCPANCR_2021-04-09_00_32_25.xlsx")
...
#MUNTEM EL CATÁLEG !!! S'ha de fer amb TOTA LA POBLACIÓ,  així agafarem TOTS ELS CODIS!!!


#i)
#dt_cmbdh_diagnostics_b<-dt_cmbdh_diagnostics%>%select(cod,agr)%>%group_by(cod)%>%dplyr::slice(1)%>%ungroup()
#ii)
#dt_diagnostics_b<-dt_diagnostics%>%select(cod,agr)%>%group_by(cod)%>%dplyr::slice(1)%>%ungroup()
#iii)
#dt_entregable_cmbdh_procediments_b<-dt_entregable_cmbdh_procediments%>%select(cod,agr)%>%group_by(cod)%>%dplyr::slice(1)%>%ungroup()
#iv)
#dt_farmacs_facturats_b<-dt_farmacs_facturats%>%select(cod,agr)%>%group_by(cod)%>%dplyr::slice(1)%>%ungroup()
#v)
#dt_farmacs_prescrits_b<-dt_farmacs_prescrits%>%select(cod,agr)%>%group_by(cod)%>%dplyr::slice(1)%>%ungroup()



#i)
#write.csv(dt_cmbdh_diagnostics_b, file="dt_cmbdh_diagnostics_b.csv")
#ii)
#write.csv(dt_diagnostics_b, file="dt_diagnostics_b.csv")
#iii)
#write.csv(dt_entregable_cmbdh_procediments_b, file="dt_entregable_cmbdh_procediments_b.csv")
#iv)
#write.csv(dt_farmacs_facturats_b, file="dt_farmacs_facturats_b.csv")
#v)
#write.csv(dt_farmacs_prescrits_b, file="dt_farmacs_prescrits_b.csv")

# MUNTEM EL CATÀLEG DEL NOSTRE ESTUDI, AMB EL nom de cataleg!
...
#comp_cat<-compar_catal(dt1=cataleg,dt2=cataleg_BOG)
...
dt_temp<-
  dt_analitiques %>% filter(cod=="TG" &  val>=500) %>% 
  group_by(idp) %>% 
    slice(which.min(dat)) %>% 
  ungroup() %>% 
...
#  mutate(year=stringr::str_sub(dat,1,4)) %>% 
#  group_by(idp,year) %>% 
#    slice(which.max(val)) %>% 
#  ungroup() %>% 
#  select(idp,dtindex=dat) 
...
dt_temp<-dt_temp %>% filter(dtindex>=params$bd.dindex1 & dtindex<=params$bd.dindex2)
...
#[xii,xiii]
###########################################################################################  
dt_variables<-dt_analitiques%>% bind_rows(dt_cliniques) %>% select(-rimap,-agr) %>% 
  semi_join(dt_index,by="idp")
########################################################################################### 
...
dt_temp<-cataleg_BOG %>% 
  filter(domini %in% c("farmacs facturats","farmacs prescrits")) %>% 
  select(cod=cod,agr=agr)
...
# dtagr_facturat_HTG<-agregar_facturacio(
#   dt=dt_farmacs_facturats,
#   bd.dindex=params$bd.dindex1,
#   finestra.dies=c(-Inf,0),
#   dt.agregadors=select(cataleg_BOG,cod=Codi,agr=Agr),
#   prefix="FF.",
#   camp_agregador="agr",
#   agregar_data=T)    
...
dt_temp<-Generar_taula_plana(dt_index,cataleg = conductor_codis2,parametres = dt_parametres)%>%select(-dtindex)
